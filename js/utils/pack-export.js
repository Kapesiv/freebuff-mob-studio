...
function geckoLibReadme(name, id, ns) {
    ...
    return `# ☕ GeckoLib Guide — ${name} (${id})

This pack contains a GeckoLib model ready for your Java mod.
Copy the files into your mod like this:
...
> NOTE: \`assets/freebuff/...\` is a vanilla resource pack item model
> (not GeckoLib) — you can ignore it in your mod.
...
// Init: FabricDefaultAttributeRegistry.register(${ClassName.toUpperCase()},
...
With a command right after the entity is registered:
\`\`\`
/summon ${ns}:${id} ~ ~ ~
\`\`\`

Natural spawning (Fabric example):
\`\`\`java
...
Generated by: Freebuff Mob Studio — ${new Date().toISOString()}
`;
}
...
function bedrockReadme(name, id, ns, hasAnims, primaryAnim, hasGlow) {
    const lines = [
        ...
        '',
        'ASENNUS (2 tapaa):',
        '  1) Avaa ladattu .mcaddon-tiedosto (kaksoisklikkaa) — Minecraft',
        '     asentaa resource_pack + behavior_pack automaattisesti.',
        '  2) Tai kopioi resource_pack/ ja behavior_pack/ -kansiot',
        '     manuaalisesti kohteisiin .../resource_packs/ ja .../behavior_packs/',
        ...
        '',
        'MUNA:',
        '  Löytyy luovasta inventaariosta (munan värit johdettu',
        '  mobin tekstuurista).',
        '',
        'ANIMAATIO:',
        `  ${hasAnims ? (primaryAnim ? 'Toistetaan automaattisesti: ' + primaryAnim : 'Animaatiotiedosto mukana.') : 'Ei animaatioita (staattinen malli).'}`,
        '',
        'HEHKU:',
        `  ${hasGlow ? 'Hehkuvat osat loistavat (entity_emissive_alpha).' : 'Ei hehkukerrosta.'}`,
        '',
        'TIEDOSTOT:',
